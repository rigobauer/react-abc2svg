...
describe('Abc2Svg', () => {
  ...
  it('shallow renders <Abc2Svg /> with default parameters', () => {
    ...
    expect(wrapper.html())
      .to.equal('<div style="width:100%"><div class="abc2svg-result" style="width:100%"></div></div>')
  })

  it('shallow renders <Abc2Svg /> with no abcNotation and showErrors option enabled', () => {
    const wrapper = shallow(<Abc2Svg showErrors />, {disableLifecycleMethods: true})
    expect(wrapper.html())
      .to.equal('<div style="width:100%"><div class="abc2svg-result" style="width:100%"></div></div>')
  })
  ...
  it('mounts <Abc2Svg /> with a simple abcNotation', () => {
    ...
    const wrapper = mount(<Abc2Svg abcNotation={'X:1\nT:Test\nM:4/4\nK:G\nAggg'} showErrors />)
