...
class Abc2svgCallbacks {
  constructor() {
    ...
    this.img_out = data => {
      this.abc_svg_output += data
    }
    this.errmsg = (msg, ln, cn) => {
...
class Abc2Svg extends PureComponent {
  ...
  fitWidth = () => {
    this.setState({
      containerWidth:
        '%%pagewidth ' + window.getComputedStyle(findDOMNode(this)).width + '\n'
    })
  }
  ...
  render() {
    ...
    abc.tosvg(
      'ABC NOTATION',
      '%%fullsvg ra2s' + this.uniqueNumber + '\n' + containerWidth + abcNotation
    )

    return (
      <div style={{ width: '100%' }}>
        <div
          className="abc2svg-result"
          style={{ width: '100%' }}
        ...
        {showErrors &&
          this.abcCallbacks.abc_error_output !== '' && (
            <div
              className="abc2svg-errors"
              style={{ width: '100%', textAlign: 'center' }}
              dangerouslySetInnerHTML={{
                __html: this.abcCallbacks.abc_error_output
              }}
            />
          )}
      </div>
